...
Цель всего этого — **гарантировать, что задачи идентифицируемые ключем будут выполняться только на конкретной ноде, конкретным актором**, это разрешает работу с Cassandra без `paxos` и поддержку ревизий.

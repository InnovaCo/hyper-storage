...
class DbSpec extends FreeSpec with Matchers with CassandraFixture
  ...
  "DbSpec" - {
    "Index collection with ordering by id" in {
      ...
      val ca = db.selectIndexCollection("index_content", "test~", "x1", Seq.empty, Seq.empty, 10).futureValue.toSeq
      ca(0).itemSegment shouldBe "i1"
      ca(1).itemSegment shouldBe "i2"
      ca(2).itemSegment shouldBe "i3"

      val cd = db.selectIndexCollection("index_content", "test~", "x1",
        Seq.empty,
        Seq(SortField("item_segment", ascending = false)),
        10).futureValue.toSeq
      cd(0).itemSegment shouldBe "i3"
      cd(1).itemSegment shouldBe "i2"
      cd(2).itemSegment shouldBe "i1"

      val ca2 = db.selectIndexCollection("index_content", "test~", "x1",
        Seq(FieldFilter("item_segment", Text("i1"), FilterGt)),
        Seq(SortField("item_segment", ascending = true)),
        10).futureValue.toSeq
      ca2(0).itemSegment shouldBe "i2"
      ca2(1).itemSegment shouldBe "i3"
      ca2.size shouldBe 2

      val cd2 = db.selectIndexCollection("index_content", "test~", "x1",
        Seq(FieldFilter("item_segment", Text("i3"), FilterLt)),
        Seq(SortField("item_segment", ascending = false)),
        10).futureValue.toSeq
      cd2(0).itemSegment shouldBe "i2"
      cd2(1).itemSegment shouldBe "i1"
      cd2.size shouldBe 2
    }
    ...
    "Index collection with ordering by text field" in {
      ...
      ca(1).itemSegment shouldBe "i2"
      ca(2).itemSegment shouldBe "i3"
      ca(3).itemSegment shouldBe "i4"

      val cd = db.selectIndexCollection("index_content_ta0", "test~", "x1",
        Seq.empty,
        Seq(SortField("t0", ascending = false)),
        10).futureValue.toSeq
      cd(0).itemSegment shouldBe "i4"
      cd(1).itemSegment shouldBe "i3"
      cd(2).itemSegment shouldBe "i2"
      cd(3).itemSegment shouldBe "i1"
      ...
      val ca3 = db.selectIndexCollection("index_content_ta0", "test~", "x1",
        Seq(FieldFilter("t0", Text("aa02"), FilterEq), FieldFilter("item_segment", Text("i3"), FilterGt)),
        Seq(SortField("t0", ascending = true), SortField("item_segment", ascending = true)),
        10).futureValue.toSeq
      ca3(0).itemSegment shouldBe "i4"
      ca3.size shouldBe 1

      val cd3 = db.selectIndexCollection("index_content_ta0", "test~", "x1",
        Seq(FieldFilter("t0", Text("aa02"), FilterEq), FieldFilter("item_segment", Text("i4"), FilterLt)),
        Seq(SortField("t0", ascending = false), SortField("item_segment", ascending = false)),
        10).futureValue.toSeq
      cd3(0).itemSegment shouldBe "i3"
      cd3.size shouldBe 1
    }
  }
}

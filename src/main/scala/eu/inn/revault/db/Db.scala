...
class Db(session: com.datastax.driver.core.Session)(implicit ec: ExecutionContext) {
  ...
  def selectContent(documentUri: String, itemSegment: String): Future[Option[Content]] = cql"""
      select document_uri,item_segment,revision,monitor_dt,monitor_channel,body,is_deleted,created_at,modified_at from content
      where document_uri=$documentUri and item_segment=$itemSegment
    """.oneOption[Content]

  def insertContent(content: Content): Future[Unit] = cql"""
      insert into content(document_uri,item_segment,revision,monitor_dt,monitor_channel,body,is_deleted,created_at,modified_at)
      values(?,?,?,?,?,?,?,?,?)
    """.bind(content).execute()

  def selectMonitor(dt: Date, channel: Int, uri: String): Future[Option[Monitor]] = cql"""
      select dt,channel,uri,revision,body,completed_at from monitor
      where dt=$dt and channel=$channel and uri=$uri
    """.oneOption[Monitor]

  def insertMonitor(monitor: Monitor): Future[Unit] = cql"""
      insert into monitor(dt,channel,uri,revision,body,completed_at)
      values(?,?,?,?,?,?)
    """.bind(monitor).execute()

  def completeMonitor(monitor: Monitor): Future[Unit] = cql"""
      update monitor set completed_at=dateOf(now())
      where dt=${monitor.dt} and channel=${monitor.channel} and uri=${monitor.uri} and revision=${monitor.revision}
    """.execute()
}
